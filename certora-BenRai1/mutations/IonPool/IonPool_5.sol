...
contract IonPool is PausableUpgradeable, RewardModule {
        ...
        onlyRole(LIQUIDATOR_ROLE)
    {
        ...
        // mutation switch _add and _sub
        _vault.collateral = _sub(_vault.collateral, changeInCollateral);
        _vault.normalizedDebt = _sub(_vault.normalizedDebt, changeInNormalizedDebt);
        ilk.totalNormalizedDebt = _sub(uint256(ilk.totalNormalizedDebt), changeInNormalizedDebt).toUint104();
        ...
        $.gem[ilkIndex][v] = _add($.gem[ilkIndex][v], changeInCollateral);
        $.unbackedDebt[w] = _add($.unbackedDebt[w], changeInDebt);
        $.totalUnbackedDebt = _add($.totalUnbackedDebt, changeInDebt);
